  ...
  <article>
    <header class="flex flex-col justify-between mt-12 items-left">
      ...
      <figure>
        ...
        <figcaption v-if="post.imgCredits" class="mt-3 text-xs text-right text-gray-600">
          Photo by
          <a v-if="post.imgCreditsUrl" :href="post.imgCreditsUrl" target="_blank" rel="noreferer noopener">
            {{ post.imgCredits }}
          </a>
          <span v-else>
            {{ post.imgCredits }}
    ...
    <div v-if="hasTags" class="my-10 border-t border-light-border dark:border-dark-border">
      <div class="flex flex-row flex-wrap justify-start my-10 light:text-light-onSurfaceSecondary dark:text-dark-onSurfaceSecondary">
        <span v-for="(tag, id) in post.tags" :key="id"
          class="px-2 py-1 mb-2 mr-2 font-medium tracking-wider uppercase truncate
            transition-colors duration-300 ease-linear
            border rounded-full text-ss border-light-border dark:border-dark-border">
          {{ tag }}
        </span>
      </div>
    </div>
